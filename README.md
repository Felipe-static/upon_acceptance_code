...
├── data/
│   ├── generacion_total.parquet          # Raw generation data (download separately — see below)
│   └── predictions_valparaiso_2023.parquet  # Pre-computed LSTM forecasts (included)
├── models/
...
**Download:** [energiaabierta.cl](https://energiaabierta.cl/) — hourly electricity generation records for all Chilean power plants (2000–2023), published by the National Energy Commission (CNE).

> If you only want to run **Stage 2** (LP optimization), no download is needed — `data/predictions_valparaiso_2023.parquet` is already included and Stage 2 can be executed independently.
